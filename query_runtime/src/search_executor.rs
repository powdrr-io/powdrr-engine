...
#[allow(dead_code)]
#[derive(Clone)]
pub struct SearchExecutionPlan {
    pub(crate) shards: Vec<SearchShardExecutionPlan>,
    pub(crate) merge: SearchMergePlan,
}

#[allow(dead_code)]
#[derive(Clone)]
pub struct SearchShardExecutionPlan {
    pub shard_id: String,
...
#[allow(dead_code)]
#[derive(Clone)]
pub enum SearchShardRoute {
    BroadcastCurrentSnapshot,
}

#[allow(dead_code)]
#[derive(Clone)]
pub enum SearchSegmentExecutionPlan {
    LegacySql(LegacySqlSegmentPlan),
}

#[allow(dead_code)]
#[derive(Clone)]
pub struct LegacySqlSegmentPlan {
...
impl SearchCommand {
    ...
    async fn private_search_invocation(&self) -> Option<PrivateSearchInvocation> {
        ...
        Some(PrivateSearchInvocation {
            ...
            sql_without_deletes: self.sql_without_deletes.clone(),
            exact_sql: self.exact_sql.clone(),
            exact_sql_without_deletes: self.exact_sql_without_deletes.clone(),
    ...
    async fn current_target_snapshots(
        ...
        match runtime_bindings::get_published_active_servable_checkpoint(&legacy_command.table)
            .await
        {
...
pub async fn execute_multi_target_search_commands(
    context: CommandContext,
...
async fn execute_private_search_invocation(
    ...
    let peer_clients = runtime_bindings::get_peer_clients().await;
...
pub async fn execute_count_command(
    command: Arc<SearchCommand>,
