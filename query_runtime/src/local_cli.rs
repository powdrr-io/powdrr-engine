...
pub async fn query_local_parquet_cache(
    ...
    runtime_bindings::set_peer_mode(&PeerModeType::SelfOnly).await;
    runtime_bindings::add_checkpoint(&checkpoint).await;
    ...
    let query_string = crate::elastic_search_api_types::QueryStringSearch {
        ...
        LocalQueryLanguage::ElasticsearchJson => {
            ...
            .and_then(|plan| {
                search_executor::search_plan_to_command(
                    plan,
                    &crate::elastic_search_api_types::QueryStringSearch::new(),
                )
            }) {
                Ok(command) => {
