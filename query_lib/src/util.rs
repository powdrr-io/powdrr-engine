...
pub(crate) fn add_file_suffix(
    base_file_path: &String,
    suffix: &String,
    extension: Option<&String>,
) -> String {
    if !base_file_path.ends_with(".json")
        && !base_file_path.ends_with(".arrow")
        && !base_file_path.ends_with(".parquet")
    {
    ...
    match index {
        Some(i) => match extension {
            None => format!(
                "{}_{}{}",
                base_file_path[..i].to_string(),
                suffix,
                base_file_path[i..].to_string()
            )
            .to_string(),
...
pub(crate) fn log_err<SuccessType, ErrorType: Error>(
    error: ErrorType,
...
pub fn log_service_err(error: ServiceApiError) -> ElasticSearchResponse {
    let error_str = format!("{}", error);
    println!("{}", error_str);
    tracing::info!("{}", error);
...
pub fn log_service_err_response(
...
pub(crate) async fn describe_table_log_error_then_none(
    table_name: &String,
