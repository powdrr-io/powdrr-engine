...
impl EphemeralFetchTracker {
                ...
                Some(value) => {
                    if value < &descriptor.checkpoint_id {
                        target_map.insert(
                            ...
                            descriptor.checkpoint_id.clone(),
                        );
                    }
                }
                None => {
    ...
    pub async fn get_next_prefetch_checkpoints(
        ...
        for (key, value) in target_map.iter() {
            match self
                .get_latest_target_checkpoint(key, extensions.clone())
                .await?
            {
                Some(latest) => {
                    if &latest < value {
                        next_prefetch_checkpoints
                            .push(CheckpointDescriptor::new(key.clone(), value.clone()))
                    }
                }
                None => next_prefetch_checkpoints
                    .push(CheckpointDescriptor::new(key.clone(), value.clone())),
            }
        }
    ...
    pub async fn set_next_prefetch_checkpoints(
        &mut self,
        table_name: &String,
        extension: Option<String>,
