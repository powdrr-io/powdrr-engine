...
pub async fn reserve(top_level_name: &String, total_size: u64, related_names: Vec<String>) -> () {
    assert!(total_size > 0);
    LRU_CACHE_HANDLE
...
pub async fn release(top_level_name: &String) -> () {
...
pub async fn load_file_as_table(
...
pub async fn load_files_as_table(
    new_local_name: &String,
    file_paths: &Vec<String>,
    schema: &Schema,
) -> Result<(), DataFusionError> {
...
pub async fn load_memtable(records: &Vec<RecordBatch>) -> Result<String, DataFusionError> {
...
pub async fn load_memtable_with_name(
    result_table_name: &String,
    records: &Vec<RecordBatch>,
) -> Result<(), DataFusionError> {
    if records.len() == 0 {
...
pub async fn execute_sql_async(sql: &String) -> Result<Vec<RecordBatch>, DataFusionError> {
...
pub async fn execute_sql(sql: &String) -> Result<DataFrame, DataFusionError> {
    assert!(
...
pub async fn create_table(table_name: &String, sql: &String) -> Result<(), DataFusionError> {
...
pub async fn drop(table_name: &String) -> () {
    LRU_CACHE_HANDLE.table_dropped(table_name).await;
}

pub async fn delete_s3_files(file_paths: &Vec<String>) -> () {
    LRU_CACHE_HANDLE.file_delete(file_paths).await
}

pub async fn put_s3_file(
    file_path: &String,
...
pub fn s3_ingest_base_path() -> String {
