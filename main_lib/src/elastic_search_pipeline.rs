...
use crate::{
    painless_parser,
    pipeline::{
        self, AppendProcessorBody, PipelineDefinition, PipelineError, PipelineProcessorBody,
        ProcessorBodies, RemoveProcessorBody, RenameProcessorBody, SetProcessorBody,
    },
...
impl<'de> Deserialize<'de> for ElasticSearchProcessorBodies {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        match only_pair.0.as_str() {
            "set" => Ok(ElasticSearchProcessorBodies::Set(
                serde_json::from_value::<SetElasticSearchProcessorBody>(only_pair.1.clone())
                    .unwrap(),
            )),
            "pipeline" => Ok(ElasticSearchProcessorBodies::Pipeline(
                serde_json::from_value::<PipelineElasticSearchProcessorBody>(only_pair.1.clone())
                    .unwrap(),
            )),
            "remove" => Ok(ElasticSearchProcessorBodies::Remove(
                serde_json::from_value::<RemoveElasticSearchProcessorBody>(only_pair.1.clone())
                    .unwrap(),
            )),
            "append" => Ok(ElasticSearchProcessorBodies::Append(
                serde_json::from_value::<AppendElasticSearchProcessorBody>(only_pair.1.clone())
                    .unwrap(),
            )),
            "rename" => Ok(ElasticSearchProcessorBodies::Rename(
                serde_json::from_value::<RenameElasticSearchProcessorBody>(only_pair.1.clone())
                    .unwrap(),
            )),
...
impl ElasticSearchPipelineDefinition {
    fn convert_if(
        ...
        match if_stmt {
            Some(if_str) => {
                ...
                Ok(Some(
                    painless_parser::translate(if_str).map_err(|_| PipelineError {})?,
                ))
            }
            None => Ok(None),
        }
    ...
    fn from_process_definition(definition: &PipelineDefinition) -> Self {
        let processors = definition
            .processors
            .iter()
            .map(|x| Self::convert_from_processor(x))
            .collect();
        let on_failure = definition
            .on_failure
            .iter()
            .map(|x| Self::convert_from_processor(x))
            .collect();
    ...
    fn convert_to_processor(
        processor: &ElasticSearchProcessorBodies,
    ) -> Result<ProcessorBodies, PipelineError> {
        match processor {
            ElasticSearchProcessorBodies::Set(body) => {
                let mut originals = HashMap::new();
                let if_translated = ElasticSearchPipelineDefinition::convert_if(
                    &body._if,
                    &"original_if".to_string(),
                    &mut originals,
                )?;
    ...
    fn to_pipeline_definition(&self) -> Result<PipelineDefinition, PipelineError> {
        let processors = self
            .processors
            .iter()
            .map(|x| Self::convert_to_processor(x))
            .collect::<Result<Vec<ProcessorBodies>, PipelineError>>()?;
        let on_failure = self
            .on_failure
            .iter()
            .map(|x| Self::convert_to_processor(x))
...
#[allow(dead_code)]
pub(crate) fn parse_named_definition(
    value: &String,
) -> Result<(String, ElasticSearchPipelineDefinition), PipelineError> {
    let named_definition =
        serde_json::from_str::<HashMap<String, ElasticSearchPipelineDefinition>>(value).unwrap();
...
) -> Result<CreatePipelineResult, ESPipelineError> {
    let pipeline_definition =
        serde_json::from_str(definition_str).map_err(|_| ESPipelineError {
            message: "dunno".to_string(),
        })?;

    STATE_PROVIDER
        .create_pipeline(name, &pipeline_definition)
        .await
        .map_err(|e| ESPipelineError {
            message: format!("{}", e),
        })?;
...
pub(crate) async fn simulate_pipeline(
    name: &Option<String>,
    definition_str: &String,
) -> Result<SimulatePipelineResult, ESPipelineError> {
    ...
    let pipeline = match request.pipeline {
        Some(pipeline) => pipeline
            .to_pipeline_definition()
            .map_err(|_| ESPipelineError {
                message: "dunno".to_string(),
            })?,
        None => match name {
            Some(name) => match STATE_PROVIDER.describe_pipeline(name).await {
                Ok(pd) => match pd {
                    Some(pd) => pd,
                    None => {
                        return Err(ESPipelineError {
                            message: "No pipeline".to_string(),
                        });
                    }
                },
                Err(e) => {
    ...
    request
        .docs
        .iter_mut()
        .for_each(|d| pipeline::apply_definition(&pipeline, d));
    let result_docs = request
        .docs
        .iter()
        .map(|x| HashMap::from([("doc".to_string(), x.clone())]))
        .collect();
...
mod tests {
    use crate::{
        elastic_search_pipeline::{ElasticSearchPipelineDefinition, parse_named_definition},
    ...
    fn double_roundtrip(original: &str) {
        ...
        let es_reparsed_def =
            serde_json::from_str::<ElasticSearchPipelineDefinition>(es_serialized.as_str())
                .unwrap();
