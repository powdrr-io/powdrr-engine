...
async fn handle_describe_table(payload: Value) -> Result<Value, DynamoDbError> {
    ...
    let mut attribute_definitions = vec![AttributeDefinition {
        attribute_name: context.config.partition_key.clone(),
        attribute_type: dynamodb_key_type(&context.schema, &context.config.partition_key)?,
    }];
    let mut key_schema = vec![KeySchemaElement {
        attribute_name: context.config.partition_key.clone(),
        key_type: "HASH",
    }];
    if let Some(sort_key) = context.config.sort_key.as_ref() {
        attribute_definitions.push(AttributeDefinition {
            attribute_name: sort_key.clone(),
            attribute_type: dynamodb_key_type(&context.schema, sort_key)?,
        });
...
async fn handle_get_item(payload: Value) -> Result<Value, DynamoDbError> {
    ...
    let response = execute_fast_path_query(
        ...
        ServingRequestPlan {
            select: projection,
            filters: key_to_predicates(&context.config, &key),
            order_by: vec![],
            limit: Some(1),
...
async fn handle_batch_get_item(payload: Value) -> Result<Value, DynamoDbError> {
    ...
    for (table_name, keys_and_attributes) in request.request_items.iter() {
        ...
        for key in keys_and_attributes.keys.iter() {
            let parsed_key = parse_key_map(key, &context.config)?;
            let response = execute_fast_path_query(
                table_name,
                ServingRequestPlan {
                    select: projection.clone(),
                    filters: key_to_predicates(&context.config, &parsed_key),
                    order_by: vec![],
                    limit: Some(1),
                    allow_slow_path: false,
                    explain: false,
...
async fn handle_query(payload: Value) -> Result<Value, DynamoDbError> {
    ...
    if request.index_name.is_some() {
        return Err(DynamoDbError::validation(
            "Secondary indexes are not supported yet",
        ));
    }
    if request.filter_expression.is_some() {
        return Err(DynamoDbError::validation(
            "FilterExpression is not supported yet",
        ));
    }
    ...
    let mut filters = vec![ServingPredicate {
        field: context.config.partition_key.clone(),
    ...
    let effective_limit = if sort_is_exact_eq || context.config.sort_key.is_none() {
        page_limit.min(1)
    } else {
    ...
    let last_evaluated_key = if rows.len() > page_limit {
        rows.truncate(page_limit);
        Some(item_to_key(&rows[rows.len() - 1], &context.config)?)
    } else {
        None
    };
...
fn derived_dynamodb_serving_patterns(config: &DynamoDbTableConfig) -> Vec<ServingPattern> {
    let mut patterns = vec![ServingPattern {
        name: format!("{}get_item", DYNAMODB_CONFIG_PATTERN_PREFIX),
        eq_fields: match config.sort_key.as_ref() {
            Some(sort_key) => vec![config.partition_key.clone(), sort_key.clone()],
            None => vec![config.partition_key.clone()],
        },
        range_field: None,
        order_field: None,
        descending: false,
    ...
    if let Some(sort_key) = config.sort_key.as_ref() {
        for descending in [false, true] {
            patterns.push(ServingPattern {
                ...
                eq_fields: vec![config.partition_key.clone()],
                range_field: None,
...
fn key_to_predicates(
    ...
    let mut predicates = vec![ServingPredicate {
        field: config.partition_key.clone(),
        eq: key.get(&config.partition_key).cloned(),
        in_values: None,
        gt: None,
        gte: None,
        lt: None,
...
fn parse_key_condition_expression(
    ...
    let partition_field = config.partition_key.clone();
    ...
    let sort_filter = match (config.sort_key.as_ref(), sort_segment) {
        (_, None) => None,
        (None, Some(_)) => {
            return Err(DynamoDbError::validation(
...
fn apply_exclusive_start_key(
    ...
    let sort_key = match config.sort_key.as_ref() {
        Some(sort_key) => sort_key,
        None => return Ok(()),
    };
    let parsed_key = parse_key_map(exclusive_start_key, config)?;
    let partition_value = parsed_key
        .get(&config.partition_key)
        .ok_or_else(|| DynamoDbError::validation("ExclusiveStartKey was missing partition key"))?;
    let partition_filter = filters
        .iter()
        .find(|filter| filter.field == config.partition_key)
...
fn item_to_key(
    ...
    key.insert(
        config.partition_key.clone(),
        item.get(&config.partition_key).cloned().ok_or_else(|| {
