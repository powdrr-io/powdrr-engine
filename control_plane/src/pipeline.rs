    ...
    fn apply(&self, value: &mut Value) -> Result<(), PipelineError> {
        match &self._if {
            Some(if_val) => {
                ...
                let if_result = expression_evaluator::eval_template(
                    if_val,
                    value,
                    HashMap::new(),
                    context! { a => "a" },
        ...
        let eval_result = expression_evaluator::eval_template(
            &self.value_formula,
            value,
            HashMap::new(),
            context! { a => "a" },
        ...
        match value {
            Value::Object(inner_val) => match inner_val.get_mut("_source") {
                Some(source_val) => match source_val {
                    Value::Object(source_map) => {
                        source_map.insert(
                            self.field.clone(),
                            Value::String(eval_result.result.as_str().to_string()),
                        );
                    }
...
impl<'de> Deserialize<'de> for ProcessorBodies {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        match only_pair.0.as_str() {
            "set" => Ok(ProcessorBodies::Set(
                serde_json::from_value::<SetProcessorBody>(only_pair.1.clone()).unwrap(),
            )),
            "rename" => Ok(ProcessorBodies::Rename(
                serde_json::from_value::<RenameProcessorBody>(only_pair.1.clone()).unwrap(),
            )),
            "pipeline" => Ok(ProcessorBodies::Pipeline(
                serde_json::from_value::<PipelineProcessorBody>(only_pair.1.clone()).unwrap(),
            )),
            "remove" => Ok(ProcessorBodies::Remove(
                serde_json::from_value::<RemoveProcessorBody>(only_pair.1.clone()).unwrap(),
            )),
            "append" => Ok(ProcessorBodies::Append(
                serde_json::from_value::<AppendProcessorBody>(only_pair.1.clone()).unwrap(),
            )),
...
pub fn apply_definition(pipeline: &PipelineDefinition, doc: &mut Value) -> () {
...
fn apply_processor(processor: &ProcessorBodies, value: &mut Value) -> Result<(), PipelineError> {
    match processor {
        ProcessorBodies::Set(body) => body.apply(value),
        ProcessorBodies::Rename(body) => body.apply(value),
        ProcessorBodies::Pipeline(body) => body.apply(value),
        ProcessorBodies::Remove(body) => body.apply(value),
        ProcessorBodies::Append(body) => body.apply(value),
    }
}
...
impl PipelineDefinition {
    ...
    pub fn new(
        description: &Option<String>,
        processors: &Vec<ProcessorBodies>,
        on_failure: &Vec<ProcessorBodies>,
    ) -> Self {
        PipelineDefinition {
            description: description.clone(),
