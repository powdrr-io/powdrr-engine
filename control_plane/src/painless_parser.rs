...
impl TranslationContext {
    ...
    fn nested_prefix_worker(nested: u32) -> String {
        match nested {
            ...
            _ => format!(
                "                    {}",
                TranslationContext::nested_prefix_worker(nested - 10)
            ),
        }
    }
}
...
trait Statement {
    fn translate(&self, context: TranslationContext) -> Result<String, TranslationError> {
        Ok(format!(
            "{}{}",
            context.nested_prefix(),
            self.translate_worker(context)?
        ))
    }
...
impl Statement for IfStatement {
    fn translate_worker(&self, context: TranslationContext) -> Result<String, TranslationError> {
        let then_str = self
            .then_statements
            .iter()
            .map(|x| x.translate(context.push()))
...
impl Statement for AssignmentStatement {
    fn translate_worker(&self, context: TranslationContext) -> Result<String, TranslationError> {
        ...
        match current_left
            .as_any()
            .downcast_ref::<VariableOrTypeReference>()
        {
            Some(l) => {
...
fn create_expression(token: &Token) -> Result<Box<dyn Expression>, TranslationError> {
    match token.kind {
        ...
        TokenKind::Identifier => Ok(Box::new(VariableOrTypeReference {
...
impl Expression for FieldAccess {
    ...
    fn translate(&self, context: TranslationContext) -> Result<String, TranslationError> {
        Ok(format!(
            "{}.{}",
            self.expression.translate(context)?,
            self.field_name
        ))
    }
}
...
impl Expression for Index {
    ...
    fn translate(&self, context: TranslationContext) -> Result<String, TranslationError> {
        Ok(format!(
            "{}[{}]",
            self.outer_expression.translate(context.clone())?,
            self.index_expression.translate(context.clone())?
        ))
    }
}
...
impl Expression for MethodInvocation {
    ...
    fn translate(&self, context: TranslationContext) -> Result<String, TranslationError> {
        ...
        match method_name.as_str() {
            ...
            _ => {
                let params_translated: Vec<String> = self
                    .params
                    .iter()
                    .map(|x| x.translate(context.clone()))
                    .collect::<Result<Vec<String>, TranslationError>>()?;
                Ok(format!(
                    "{}({})",
...
impl Expression for Comparison {
    ...
    fn translate(&self, context: TranslationContext) -> Result<String, TranslationError> {
        ...
        let is_left_field_access = self
            .left_expression
            .as_any()
            .downcast_ref::<FieldAccess>()
            .is_some();
        let left_expression_str = self.left_expression.translate(context.clone())?;
        if self.operator == "!=" && is_right_null {
            if is_left_field_access {
                Ok(format!(
                    "({left_expression_str} is defined and {left_expression_str} is not none)"
                ))
            } else {
                Ok(format!("{left_expression_str} is not none"))
            }
        } else if self.operator == "==" && is_right_null {
            if is_left_field_access {
                Ok(format!(
                    "({left_expression_str} is not defined or {left_expression_str} is none)"
                ))
            } else {
                Ok(format!("{left_expression_str} is none"))
            }
        } else {
            Ok(format!(
                "{left_expression_str} {} {}",
...
fn create_conditions_str(
    outer_expression_str: &String,
    nested_field_chain: &Vec<String>,
) -> String {
    let field_chains = create_field_chains(nested_field_chain);

    let mut condition_str: Vec<String> = field_chains
        .iter()
        .map(|x| create_field_access(x))
        .map(|x| {
            format!(
                "{}{} is defined and {}{} is mapping",
                outer_expression_str, x, outer_expression_str, x
            )
        })
        .collect::<Vec<String>>();
    condition_str.push(format!(
        "{} is defined and {} is mapping",
        outer_expression_str, outer_expression_str
    ));
    ...
    fn translate(&self, context: TranslationContext) -> Result<String, TranslationError> {
        ...
        loop {
            match test_expression
                .expression
                .as_any()
                .downcast_ref::<NullTestFieldAccess>()
            {
                Some(ntfa) => {
        ...
        Ok(format!(
            "({}{} if {} else none)",
            expression_str, field_access_str, conditions_str
        ))
    }
}

pub fn translate(painless_script: &String) -> Result<String, TranslationError> {
    ...
    let translated_statements: Result<Vec<String>, TranslationError> = statements
        .iter()
        .map(|s| s.translate(translation_context.clone()))
        .collect();
...
) -> Result<Box<dyn Statement>, TranslationError> {
    ...
    match token.kind {
        ...
        TokenKind::Identifier => {
            ...
            match next_token.kind {
                ...
                TokenKind::Identifier => parse_variable_declaration(parser_context, &token),
            }
        }
    }
}
...
fn parse_expression_booleans(
    ...
    while parser_context.has_more() {
        match parser_context.peek().value.as_str() {
            "&&" => {
                parser_context.pop_validate("&&");
                expression = Box::new(Comparison::new(
                    expression,
                    "and".to_string(),
                    parse_expression_comparisons(parser_context)?,
                ));
            }
            "||" => {
                parser_context.pop_validate("||");
...
fn parse_expression_comparisons(
    parser_context: &mut ParserContext,
) -> Result<Box<dyn Expression>, TranslationError> {
    ...
    while parser_context.has_more() {
        match parser_context.peek().value.as_str() {
            "<" => {
                expression = Box::new(Comparison::new(
                    expression,
                    parser_context.pop().value,
                    parse_expression_mut_div(parser_context)?,
                ));
            }
            "<=" => {
                expression = Box::new(Comparison::new(
        ...
        match parser_context.peek().value.as_str() {
            "*" => {
                expression = Box::new(Comparison::new(
                    expression,
                    parser_context.pop().value,
                    parse_expression_add_sub(parser_context)?,
                ));
            }
            "/" => {
                expression = Box::new(Comparison::new(
        ...
        match parser_context.peek().value.as_str() {
            "+" => {
                expression = Box::new(Comparison::new(
                    expression,
                    parser_context.pop().value,
                    parse_expression_inner_most(parser_context)?,
                ));
            }
            "-" => {
                expression = Box::new(Comparison::new(
...
fn parse_expression_inner_most(
    parser_context: &mut ParserContext,
) -> Result<Box<dyn Expression>, TranslationError> {
...
fn parse_variable_declaration(
    _parser_context: &mut ParserContext,
    _first_token: &Token,
) -> Result<Box<dyn Statement>, TranslationError> {
...
fn parse_field_access(
    ...
    Ok(Box::new(FieldAccess {
        expression: expression,
        field_name: field_name.value.clone(),
    }))
}

fn parse_null_test_field_access(
    parser_context: &mut ParserContext,
    expression: Box<dyn Expression>,
) -> Result<Box<dyn Expression>, TranslationError> {
    parser_context.pop_validate("?.");
    ...
    Ok(Box::new(NullTestFieldAccess {
        expression: expression,
        field_name: field_name.value.clone(),
    }))
}
...
mod tests {
    ...
    fn test_tokenizer() {
        let context =
            ParserContext::new(&"ctx?.kibana?.log?.meta?.res?.responseTime != null".to_string());
    ...
    fn test_translate_datetime_method() {
        ...
        assert_eq!(
            translated,
            r#"{{ __types.ZonedDateTime.parse("2025-05-31T12:34:56").toInstant().toEpochMilli() }}"#
        );
    }
