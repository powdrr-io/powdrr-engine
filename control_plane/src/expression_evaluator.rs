    ...
    fn new() -> Self {
        Types {
            types: HashMap::from([(
                ...
                Value::from_object(ZonedDateTimeClass {}),
            )]),
        }
    }
}
    ...
    fn call_method(
        ...
        if name == "parse" {
            if args[0].is_undefined() {
                Ok(Value::from_object(ZonedDateTimeObject {
                    value: DateTime::from_timestamp_millis(0).unwrap().fixed_offset(),
                }))
            } else {
                let arg_str = args[0].as_str().unwrap().to_string();
                Ok(Value::from_object(ZonedDateTimeObject {
                    value: DateTime::parse_from_rfc3339(&arg_str).unwrap(),
                }))
            }
        } else {
            panic!("Method does not exist in Outputs")
...
fn get_container<'a>(
    ...
    (
        current_obj,
        path_split.get(current_index).unwrap().to_string(),
    )
}
...
pub fn apply(
    ...
    for (path, value) in updates {
        ...
        if path_split.len() == 2 {
            ...
            assert_eq!(
                path_split.get(1).unwrap().to_string(),
                "_source".to_string()
            );
...
mod tests {
    ...
    fn test_assignment_script() {
        ...
        let eval_result = eval_template(
            translated.as_str(),
            &source_val,
            HashMap::new(),
            Value::from_serialize(params_val),
        );
    ...
    fn test_translate_and_execute_datetime_method() {
        ...
        let eval_result = eval_template(
            ...
            HashMap::new(),
            Value::from_safe_string("nope".to_string()),
        );
