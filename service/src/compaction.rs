...
async fn do_speedboat_commit(table_name: &String, file_path: &String, compaction_id: &String, num_records: u32) -> Result<(), RecvError> {
...
pub(crate) async fn perform_compaction(work_items: Vec<TableMetadataCheckpoint>, last_snapshot_id: i64) -> Result<i64, CompactionError> {
    ...
    for table_group in table_groups.iter() {
        ...
        let files = collect_speedboat_files(table_group.1);
        let snapshot_ids: Vec<String> = table_group.1.iter().map(|c| c.checkpoint_id.clone()).collect();
        ...
        match compact_logs(
            ...
            Ok(result) => match result {
                ...
                CompactionResult::Iceberg{ num_records} => {
                    ...
                    match do_iceberg_commit(&table_group.0, last_snapshot_id).await {
                ...
                CompactionResult::Speedboat{ file_location, num_records } => {
                    ...
                    match do_speedboat_commit(&table_group.0, &file_location, &compaction_id, num_records.try_into().unwrap()).await {
                        Ok(_) => (),
